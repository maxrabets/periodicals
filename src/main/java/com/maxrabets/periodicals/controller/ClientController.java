...
//	}
    ...
    public ResponseEntity<?> clearCookies(@CookieValue(value="token", defaultValue="") String token, HttpServletRequest request, HttpServletResponse response){
    ...
    public ResponseEntity<?> deleteClient(@CookieValue(value="token", defaultValue="") String token, HttpServletRequest request, HttpServletResponse response){
    	if(token == null || token.isEmpty()) {
    		System.err.println("null");
    		return new ResponseEntity<>(HttpStatus.FORBIDDEN);
    	}
			...
			if(userService.deleteByEmail(login)) {    		
				return clearCookies(token, request, response);
			}
			else
